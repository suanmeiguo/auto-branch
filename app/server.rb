...
class GHAapp < Sinatra::Application
  ...
  post '/create-branch-from-issue/event_handler' do
    ...
    when 'issues'
      if @payload['action'] === 'assigned'
        handle_issue_assigned_event(@payload)
      end
    end
  ...
  helpers do
    ...
    def cleanup_branch_name(branch_name)
      return branch_name
    end
    
    # handle issue assigned
    def handle_issue_assigned_event(payload)
      repo = payload['repository']['full_name']
      issue_number = payload['issue']['number']
      issue_title = payload['issue']['title']
      default_branch = payload['repository']['default_branch']
      
      branch_info = @installation_client.branch(repo, default_branch)
      sha = branch_info["commit"]["sha"]
      
      resp = @installation_client.create_ref(repo, "refs/heads/i#{issue_number}", sha)
      logger.info(resp)
